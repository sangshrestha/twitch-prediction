...
console.info(`Starting prediction with blu win percent set to ${bluPercent}`);
bluInput.blur();
...
function gamba(mutations, observer) {
  ...
  const bluBetAdjusted =
    bluPotAmt === 0 ? 1 : adjustBet(bluEvenBet, bluOdds, bluPercent, bluPotAmt);
...
function adjustBet(evenBet, odds, percent, pot) {
  ...
  const kellyMultiplier = (odds * winProbability - (1 - winProbability)) / odds;
  console.log(kellyMultiplier);
  return Math.round(
    Math.min(
      MAX_BET,
      pot * 10,
      evenBet * 0.5,
      Math.max(balance * Math.min(kellyMultiplier, 0.2), Math.min(balance, 50)),
    ),
  );
}
